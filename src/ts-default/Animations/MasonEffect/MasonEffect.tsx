...
interface MasonEffectProps {
  ...
  onReady?: (instance: { particles: Particle[]; canvas: HTMLCanvasElement }) => void;
  onUpdate?: (instance: { particles: Particle[]; canvas: HTMLCanvasElement }) => void;
}

export interface MasonEffectHandle {
  morph: (textOrOptions?: string | Partial<MasonEffectProps>) => void;
  scatter: () => void;
  updateConfig: (config: Partial<MasonEffectProps>) => void;
  getInstance: () => { particles: Particle[]; canvas: HTMLCanvasElement | null };
}
...
const MasonEffect = forwardRef<MasonEffectHandle, MasonEffectProps>(
  (
    {
      className = '',
      text = 'MASON',
    ...
    const buildTargets = () => {
      ...
      const fontSize = findOptimalFontSize(offCtx, configRef.current.text, maxWidth, maxHeight, initialFontSize);
    ...
    const debouncedResize = useRef<((...args: any[]) => void) | null>(null);
    ...
    useImperativeHandle(ref, () => ({
      morph: (textOrOptions?: string | Partial<MasonEffectProps>) => {
      ...
      updateConfig: (newConfig: Partial<MasonEffectProps>) => {
    ...
    return (
      <div ref={containerRef} className={`mason-effect-container ${className}`}>
